...
public class LogAspect {
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        try {
            logger.info("interface request,url:" + request.getRequestURL().toString() + ",param:" + getParamJsonString(joinPoint));
        } catch (Exception e) {
            //记录本地异常日志
            logger.error("==前置通知异常==");
            logger.error("异常信息:{}", e);
    ...
    public void doAfterReturning(JoinPoint joinPoint, Object returnValue) {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        try {
            logger.info("interface response,url:" + request.getRequestURL().toString()
    ...
    public void doAfterThrowing(JoinPoint joinPoint, Exception e) {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        try {
            logger.error("interface error,url:" + request.getRequestURL().toString()
